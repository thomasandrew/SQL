...
SELECT DISTINCT estado 
   FROM cliente
  ORDER BY 1; ---Perguntar pro professor
...
---EX14
SELECT nomecliente, EXTRACT(YEAR FROM age (dtnascimento)) AS idade
  FROM cliente
 WHERE EXTRACT (YEAR FROM age(dtnascimento)) > 20
 ORDER BY idade;
...
SELECT nomecliente, estadocivil, estado
  FROM cliente 
 WHERE estado IN ('Rio de Janeiro', 'Minas Gerais', 'São Paulo');
...
SELECT estadocivil FROM cliente;
